...
final class OperationHelper {
    ...
    static BsonDocument getDocumentKey(final BsonDocument changeStreamDocument) {
        if (!changeStreamDocument.containsKey(DOCUMENT_KEY)) {
            ...
            changeStreamDocument.computeIfPresent(DOCUMENT_KEY, (k, v) -> {
                try {
                    return BsonDocument.parse(v.asString().getValue());
                } catch (Exception e) {
                    throw new DataException("Invalid JSON string for DOCUMENT_KEY", e);
                }
            });
    ...
    static BsonDocument getFullDocument(final BsonDocument changeStreamDocument) {
        if (!changeStreamDocument.containsKey(FULL_DOCUMENT)) {
            ...
            changeStreamDocument.computeIfPresent(FULL_DOCUMENT, (k, v) -> {
                try {
                    return BsonDocument.parse(v.asString().getValue());
                } catch (Exception e) {
                    throw new DataException("Invalid JSON string for FULL_DOCUMENT", e);
                }
            });
